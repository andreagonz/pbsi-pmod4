...
def hacer_peticion(url, proxy, user_agent, intervalo=0, post=False, data=None, sesion=None):
    try:
        s = sesion if sesion else requests
        time.sleep(intervalo)
        headers = {'User-Agent': user_agent}
        if post:
            # return requests.post(url, headers=headers, proxies=proxy, data=data, verify=False)
            return s.post(url, headers=headers, proxies=proxy, data=data)
        # return requests.get(url, headers=headers, proxies=proxy, verify=False)
        return s.get(url, headers=headers, proxies=proxy)
